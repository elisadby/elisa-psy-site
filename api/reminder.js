...
function buildReminderEmail({ prenom, dateStr, timeStr, meetLink }) {
  const accent = '#C48A71';
  const border = '#E8E2D9';
  const muted  = '#6B6560';
  const text   = '#2B2927';
  ...
  return `<!DOCTYPE html>
<html lang="fr">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0"><title>Rappel rendez-vous</title></head>
  ...
  for (const key of allKeys) {
    ...
    try {
      ...
      await sendBrevoEmail({
        ...
        html: buildReminderEmail({ prenom: booking.prenom, dateStr, timeStr, meetLink })
      });
